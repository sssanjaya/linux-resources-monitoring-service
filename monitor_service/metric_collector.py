...
class MetricCollector:
    ...
    def _measure_performance(self, func_name: str, func):
        ...
        try:
            ...
            if self.memory_tracker:
                ...
                self.logger.debug(f"{func_name} took {execution_time:.3f}s, used {memory_used} bytes")
    ...
    def collect_cpu_metrics(self):
        ...
        if result is None:
            self.logger.error(f"Error collecting CPU metrics after {execution_time:.3f}s")
            return None
    ...
    def collect_memory_metrics(self):
        ...
        result, execution_time = self._measure_performance('memory_metrics', _collect_memory)
        
        if result is None:
            self.logger.error(f"Error collecting memory metrics after {execution_time:.3f}s")
            return None
        ...
        def _collect_disk():
            ...
            for partition in psutil.disk_partitions():
                try:
                    ...
                    self.logger.warning(f"No permission to access {partition.mountpoint}")
                    continue
                except Exception as e:
                    # Log error for other partition access issues
                    self.logger.error(f"Error accessing partition {partition.device}: {e}")
                    continue
            
            return disk_data
        
        result, execution_time = self._measure_performance('disk_metrics', _collect_disk)
        
        if result is None:
            self.logger.error(f"Error collecting disk metrics after {execution_time:.3f}s")
            return None
    ...
    def _log_final_statistics(self):
        ...
        # Calculate runtime
        runtime = self.performance_stats['shutdown_time'] - self.performance_stats['start_time']
        ...
        if 'avg_cpu_time' in stats:
            self.logger.info(f"Average CPU Collection Time: {stats['avg_cpu_time']:.3f}s")
        if 'avg_memory_time' in stats:
            self.logger.info(f"Average Memory Collection Time: {stats['avg_memory_time']:.3f}s")
        if 'avg_disk_time' in stats:
            self.logger.info(f"Average Disk Collection Time: {stats['avg_disk_time']:.3f}s")
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'error_rate': (self.performance_stats['errors'] / max(self.performance_stats['total_collections'], 1)) * 100
        }
        
        # Calculate averages
        if self.performance_stats['cpu_collection_times']:
            stats['avg_cpu_time'] = sum(self.performance_stats['cpu_collection_times']) / len(self.performance_stats['cpu_collection_times'])
        if self.performance_stats['memory_collection_times']:
            stats['avg_memory_time'] = sum(self.performance_stats['memory_collection_times']) / len(self.performance_stats['memory_collection_times'])
        if self.performance_stats['disk_collection_times']:
            stats['avg_disk_time'] = sum(self.performance_stats['disk_collection_times']) / len(self.performance_stats['disk_collection_times'])
